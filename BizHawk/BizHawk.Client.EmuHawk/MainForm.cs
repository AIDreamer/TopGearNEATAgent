	...
	public partial class MainForm : Form
	{
		...
		public int ProgramRunLoop()
		{
			...
			for (;;)
			{
                ...
                if (serverControl && (Global.Game.Name == gameTraining))
                {
                    if (loopCount == 3)
                    {
                    ...
                    Global.ActiveController.Overrides(Global.ServerAndAdaptor);
                }
				StepRunLoop_Core();
				StepRunLoop_Throttle();

                Render();
                // TODO: Bucknell AI, push stuffs to server here
                if (training && (Global.Game.Name == gameTraining))
                {
                    if (seperatePixels)
                    {
                        if (loopCount == 7)
                        {
                            PushScreenShotToPort();
                            ToggleBG_OBJ();
                    ...
                    else
                    {
                        ...
                        if (!screenSet)
                        {
                            screenPost.PixelsBG0 = null;
                            screenPost.PixelsBG3 = null;
                            screenPost.PixelsOBJ3 = null;

                            var s = AsSNES.GetSettings();
                            //Turn off all unnecessary settings
                            s.ShowBG1_0 = s.ShowBG1_1 = false;
                            s.ShowBG2_0 = s.ShowBG2_1 = false;
                            s.ShowBG4_0 = s.ShowBG4_1 = false;

                            s.ShowOBJ_0 = false;
                            s.ShowOBJ_1 = false;
                            s.ShowOBJ_3 = false;

                            //Switch the layer on/off accordingly
                            s.ShowBG3_0 = s.ShowBG3_1 = true;
                            s.ShowOBJ_2 = true;
                ...
                //Syncing the screen push and control pull
                if ((loopCount >= 9) || ((seperatePixels) && loopCount >= 5))
                {
                    loopCount = 0;
                }
                else
        ...
        //Bucknell AI Specific Settings
        public string gameTraining = ConfigurationManager.AppSettings["gameTraining"];
        public string saveStatePath = ConfigurationManager.AppSettings["saveStatePath"];
        public Boolean training = ConfigurationManager.AppSettings["training"] == "1" ? true : false;
        public Boolean serverControl = ConfigurationManager.AppSettings["serverControl"] == "1" ? true : false;
        public Boolean seperatePixels = ConfigurationManager.AppSettings["seperatePixels"] == "1" ? true : false;

        int getBG = 2;
        string[] buttons = { "P1 A", "P1 B", "P1 X", "P1 Y", "P1 Up", "P1 Down", "P1 Left", "P1 Right", "P1 L", "P1 R" };
        

        WebClient webClient = new WebClient();
        DataContractJsonSerializer jsonSerializer = new DataContractJsonSerializer(typeof(ScreenPost));
        ScreenPost screenPost = new ScreenPost();
        //Bucknell AI Stuffs, Push the current screen to server
        public void PushScreenShotToPort()
        {
            
            //Screenshot stuffs
            BitmapBuffer bb;

            bb = MakeScreenshotImage();
            if (seperatePixels)
            {
                screenPost.Pixels = null;
                if (getBG == 1)
                {
                    for (int i = 0; i < bb.Pixels.Length; i++)
                    {
                        screenPost.PixelsBG3[i] = bb.Pixels[i];
                    }
                ...
                else if (getBG == 2)
                {
                    for (int i = 0; i < bb.Pixels.Length; i++)
                    {
                        screenPost.PixelsOBJ3[i] = bb.Pixels[i];
                    }
                }
                else
                {
                    for (int i = 0; i < bb.Pixels.Length; i++)
                    {
                        screenPost.PixelsBG0[i] = bb.Pixels[i];
                    }
                }
            } else
            {
                ...
                for (int i = 0; i < bb.Pixels.Length; i++)
                {
                    screenPost.Pixels[i] = bb.Pixels[i];
                }
            }

            // Speed (multiplie by about 65)
            screenPost.Speed = ReadUnsignedLittle(0x001E5A, 2, "WRAM");
            // Go to some really high number when nitro is active
            screenPost.Nitro = ReadUnsignedLittle(0x001E6C, 1, "WRAM");
            // This number + 1 to get on screen rank
            screenPost.Rank = ReadUnsignedLittle(0x001E7A, 1, "WRAM");
            screenPost.Time = ReadUnsignedLittle(0x001EDA, 2, "WRAM");
            // We started behind the goal so distance is large
            screenPost.Distance = ReadUnsignedLittle(0x001ED8, 1, "WRAM");
            screenPost.Lap = ReadUnsignedLittle(0x00011E, 1, "WRAM");

            //Set up the web related stuffs
            using (MemoryStream ms = new MemoryStream())
            {
                jsonSerializer.WriteObject(ms, screenPost);
                webClient.UploadString("http://localhost:37979/screen", Encoding.Default.GetString(ms.ToArray()));
            }

            
        }

        public void ToggleBG_OBJ()
        {
            if (getBG == 2)
            {
                getBG = 0;
            ...
            //Switch the layer on/off accordingly
            s.ShowBG3_0 = s.ShowBG3_1 = (getBG == 1);
            s.ShowOBJ_2 = (getBG == 2);
            AsSNES.PutSettings(s);
        ...
        public void GetControlsFromPort()
        {
            string control = webClient.DownloadString("http://localhost:37979/control");
            if (control[control.Length - 1] == '1')
            {
                LoadQuickSave(saveStatePath, false, true);
                return;
            }
            for (int i = 0; i < buttons.Length; i++)
            {
                var pressed = control[i] == '1' ? true : false;
                Global.ServerAndAdaptor.SetButton(buttons[i], pressed);
            }
        ...
        public uint ReadUnsignedLittle(int addr, int size, string domain = null)
        {
            ...
            var memory = AsSNES.MemoryDomains[domain];
            uint v = 0;
            for (int i = 0; i < size; ++i)
            {
                v |= ((uint)memory.PeekByte(addr + i)) << (8 * i);
            }
